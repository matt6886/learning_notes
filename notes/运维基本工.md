...
在命令模式下按下i或者a，则进入编辑模式

命令模式+i：在光标之前插入内容， intert

命令模式+a： 在光标之后插入内容： append
...
> root用户和普通用户修改密码有点区别：
>
> * root用户 su root 可以直接设置密码
> * 普通用户su zs 需要先输入之前的密码，然后才能重置密码
...
案例：/tmp文件夹，拥有最高全系爱你777，比如用户a创建了一个文件在此目录，用户b也可以对这个文件进行删除操作，这种显然不太合适。
...
运行级别全程（Running Level），代表Linux系统的不同运行模式。
...
公司开发了一个电商网站，由于访问量很大，网络后端由100台服务器组成集群。50台服务器负责接受订单，50台服务器负责安排发货，接收订单的服务器需要记录下用户下单的具体时间，把数据传给负责发货的服务器，由于100台服务器的时间各不相同，记录的时间京城不一致，甚至会出现下单时间是明天，发货时间是昨天的情况。
...
```bash
*/3 0 18-23 * * * /usr/bin/systemctl restart network
```
...
暗流5:每1分钟往root家目录中的readme.txt中输出一个1，为了看到效果使用追加输出，输出使用echo命令，echo 输出的内容
...
crontab时任何用户都可以创建的任务，但是超级管理员可以通过配置设设置哪些用户不允许计划任务。
...
# ps -ef | grep 要查看的进程名称
ps -ef | grep crond
matt        3583    3325  0 02:06 pts/1    00:00:00 grep --color=auto crond
ps -ef | grep sshd
